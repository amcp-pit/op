...
void _merge(int* M, size_t first, size_t middle, size_t last){
...
void mergeSort(int* M, size_t size){
	if (size < 2) return;
...
void mergeSortA(int * M, size_t size){
	if (size<2) return;
#ifdef SORTSTAT
	clock_t ticktok = clock();
#endif	
...
void heapSort(int* M, size_t size){
	if (size<2) return;
