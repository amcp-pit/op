...
/*
				if (from[start1] < from[start2]){
					to[indexto] = from[start1];
					++start1;
				} else {
                    to[indexto] = from[start2];
                    ++start2;
				}
				++indexto;
*/
...
void heapSort(T * M, size_t size){
	...
	// shift down each element O(n * log(n))
	size_t N = size;
	for(size_t i=1; i < size; ++i){
		--N;
		std::swap(M[0], M[N]);
...
#ifdef SORTSTAT
	, unsigned long long &count_set, unsigned long long &count_less
#endif
){
	size_t posMaxChild;
...
#ifdef SORTSTAT
        shift_down(M, size, i-1, count_set, count_less);
#else
        shift_down(M, size, i-1);
#endif
	}
	// shift down each element O(n * log(n))
	size_t N = size;
...
#ifdef SORTSTAT
		shift_down(M, N, 0, count_set, count_less);
#else
		shift_down(M, N, 0);
#endif
