...
/*
Добавление к классу Auto_ptr1 семантики перемещения.
*/
template<class T>
class Auto_ptr2 {
	T* m_ptr;
...
/*
std::auto_ptr, представленный в C++98, был первой попыткой в языке C++ сделать 
стандартизированный умный указатель. В std::auto_ptr решили реализовать 
семантику перемещения точно так же, как это сделано в классе Auto_ptr2. 
Проблемы std::auto_ptr:
- так как реализует семантику перемещения через конструктор копирования
  и оператор присваивания, то передача std::auto_ptr в функцию по значению 
  приведет к тому, что Item будет перемещен в параметр функции и, следовательно, 
  будет уничтожен в конце функции, когда параметры этой функции выйдут из области видимости. 
- всегда удаляет свое содержимое, используя оператор delete, который не работает с массивами. 

в C++11 перестали использовать std::auto_ptr, а в C++17 удалили из Стандартной библиотеки С++.
*/
